...
contract("StakingService; Group: Staking", (accounts) => {
  ...
  before(async () => {
    ...
    stakingRouter = await ConstantComplexityStakingRouter.new(nmx.address);
    ...
    await nmx.transfer(accounts[1], toWei(toBN(1000)));
    await nmx.approve(stakingService.address, toWei(toBN(500)), {
      from: accounts[1],
    });
    await nmx.transfer(accounts[3], toWei(toBN(100)));
    await nmx.approve(stakingService.address, toWei(toBN(50)), {
      from: accounts[3],
    });
    await nmx.transfer(accounts[4], toWei(toBN(50)));
    await nmx.approve(stakingService.address, toWei(toBN(100)), {
      from: accounts[4],
    });
  });
  ...
  function verifyStakingServiceBalance(balance) {
    step(`StakingService balance is "${balance}"`, async () => {
      assertBN(
        await nmx.balanceOf(stakingService.address),
        balance,
