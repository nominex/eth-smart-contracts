...
contract("StakingService2#claimReward", (accounts) => {
  ...
  before(async () => {
    ...
    await nmx.transfer(accounts[3], toWei(toBN(500)));
    await nmx.approve(stakingService.address, toWei(toBN(500)), {
      from: accounts[3],
    });
  ...
  async function stake(amount) {
    await nmx.transfer(user, toWei(toBN(amount)));
    await nmx.approve(stakingService.address, toWei(toBN(amount)), {
      from: user,
    });
    await stakingService.stakeFrom(user, toWei(toBN(amount)));
  }

  async function unstake(amount) {
    await stakingService.unstake(toWei(toBN(amount)), { from: user });
    await nmx.transfer(nmx.address, toWei(toBN(amount)), { from: user });
  }
...
contract("StakingService2#claimWithAuthorization", async (accounts) => {
  ...
  before(async () => {
    ...
    await nmx.transfer(rewardOwner, toWei(toBN(10)));
    await nmx.approve(stakingService.address, toWei(toBN(10)), {
      from: rewardOwner,
    });
