...
contract("Nmx - initializing", (accounts) => {
  ...
  it("already distributed nmx minted to sender address", async () => {
    ...
    assert(
      balance.gt(ZERO),
      `Unexpected initial balance of the sender ${balance}`
    );
  });
...
contract("Nmx - transfer pool ownership", (accounts) => {
  ...
  it("transfer to current same pool owner fails", async () => {
    ...
    try {
      ...
      assert(
        e.message.includes("NMX: new owner must differs from the old one"),
        `Unexpected error message: ${e.message}`
      );
    }
  });
  ...
  it("arbitrary sender can not transfer ownership", async () => {
    try {
      ...
      assert(
        e.message.includes("NMX: only owner can transfer pool ownership"),
        `Unexpected error message: ${e.message}`
      );
    }
  });
  ...
  it("can not transfer ownership to owner of another pool", async () => {
    ...
    try {
      ...
      assert(
        e.message.includes("NMX: every pool must have dedicated owner"),
        `Unexpected error message: ${e.message}`
      );
    }
  });
...
contract("Nmx - supplyNmx", (accounts) => {
  ...
  it("pool owner actually got nmx on balance", async () => {
    ...
    assert(
      rewardRate.gt(ZERO),
      `Unexpected rewardRate for poolOwner ${rewardRate}`
    );
    assert(
      finalBalance.sub(initialBalance).eq(rewardRate),
      `balance change does not match the rewardRate ${initialBalance} ${finalBalance} ${rewardRate}`
    );
  });
  ...
  it("another pool state does not change", async () => {
    ...
    assert.deepEqual(
      finalState,
      initialState,
      "Another pool state was changed"
    );
  });
});
...
contract("Nmx - rewardRate", (accounts) => {
  ...
  it("primary pool are used to calc the rate", async () => {
    const expectedRate = (await nmx.poolMintStates(1)).nextTickSupply.mul(
      web3.utils.toBN(4)
    );
    const rewardRate = await nmx.rewardRate();
    assert(
      rewardRate.eq(expectedRate),
      `Reward rate ne to nextTickSupply*4. Probably wrong pool was passed ${expectedRate} ${rewardRate}`
    );
  });
});
...
contract("Nmx - permit", async (accounts) => {
  ...
  it("permit success with correct amount", async () => {
    ...
    assert(
      web3.utils
        .toBN(permitInfo.value)
        .eq(await nmx.allowance(accounts[0], accounts[1]))
    );
  });
  ...
  it("expired deadline", async () => {
    ...
    try {
      ...
      assert(
        e.message.includes("NMX: deadline expired"),
        `Unexpected error message: ${e.message}`
      );
    }
  });
  ...
  it("incorrect nonce", async () => {
    ...
    try {
      ...
      assert(
        e.message.includes("NMX: invalid signature"),
        `Unexpected error message: ${e.message}`
      );
    }
  });
