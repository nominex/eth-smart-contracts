...
contract('StakingService; Group: NmxSupplier', (accounts) => {
    ...
    before(async () => {
        ...
        stakingService = await StakingService.new(accounts[1], stakingToken.address, accounts[3]);
    });
    ...
    function checkSupplier(expectedAddress) {
        step(`Check that current nmxSupplier is ${expectedAddress}`, async () => {
            assert.equal(expectedAddress, await stakingService.nmxSupplier(), "nmxSupplier");
        });
    }
    ...
    function checkOwner(address) {
        step(`Check that owner is ${accounts[0]}`, async () => {
            assert.equal(address, await stakingService.owner(), "stakingService owner");
        });
    }

    function checkNotOwner(address) {
        step(`Check that ${accounts[0]} is not owner` , async () => {
            assert.notEqual(address, await stakingService.owner(), "stakingService owner");
        });
    }
