...
contract('StakingService; Group: Staking', (accounts) => {
    ...
    function checkStakedEventEmitted(staker, amount) {
        step(`Check "Staked" event emitted with params: owner=${staker}, amount=${amount}`, async () => {
            truffleAssert.eventEmitted(tx, 'Staked', (ev) => {
                return ev.owner === staker && fromWei(ev.amount) === amount.toString();
            });
        });
    }
    ...
    function checkUnstakedEventEmitted(staker, to, amount) {
        step(`Check "Unstaked" event emitted with params: from=${staker}, to=${to}, amount=${amount}`, async () => {
            truffleAssert.eventEmitted(tx, 'Unstaked', (ev) => {
                return ev.from === staker && ev.to === to && fromWei(ev.amount) === amount.toString();
            });
        });
    }
